...
def start_training(dataset_name: str, model_name: str, epochs: int, learning_rate: float) -> dict[str, Any]:
    return request(
        "POST",
        "/felix/training-jobs",
        json={
...
def main() -> None:
    ...
    if not state.get("generation_job_id"):
        generation = start_generation(args.dataset_name, args.num_examples)
        state["generation"] = generation
        state["generation_job_id"] = generation.get("job_id") or generation.get("id")
        save_state(state)
        print(f"Started generation job: {state['generation_job_id']}")

    generation_ready = False
    if args.poll and state.get("generation_job_id"):
        generation_status = poll_generation(state["generation_job_id"], args.interval, args.timeout)
        ...
        generation_ready = state.get("generation_status", {}).get("status") == "ready"
    ...
    if not state.get("training_job_id"):
        training = start_training(args.dataset_name, args.model_name, args.epochs, args.learning_rate)
